...
def clean_df(df):
    ...
    if len(df.columns) < NUM_OF_COLUMNS:
        response += "Please provide all required columns."
        return df, False, response

    # check if file has header first:
    has_header, new_response = check_df_has_header(df)
    response += new_response
    if not has_header:
        return df, has_header, response

    # rename columns to expected requirements:
    df, valid, new_response = check_column_names(df)
    response += new_response
    ...
    # change data types:
    df, valid, new_response = change_column_datatypes(df)
    response += new_response
    if not valid:
        return df, valid, response
...
def clean_full_df(df):
    ...
    df, new_response, valid = check_missing_values(df)
    response += new_response
    ...
    if not valid:
        ...
        df, new_response, valid = handle_date_gap(df)
        response += new_response
